 ...
 */
public class StrategyForId implements Strategy {

    /**
     * this method create a new list with the Sales between two dates
     *
     * @param parameter, the id for decides de new list
    ...
    public ArrayList<SaleModel> order(Object... parameter) {
        ...
        for (int i = 0; i < allSales.size(); i++) {
            ...
            if (allSales.get(i).getUser().getId().equals(comparator)) {
                filtradas.add(allSales.get(i));
            }
        }
        if (filtradas.isEmpty()) {
                try {
                    throw new CustomException("No hay subastas para esta persona establecida");
                } catch (CustomException ex) {
                    System.out.println(ex.getMessage());
                }
            }
        
        filtradas.sort(new OrderForId());
        return filtradas;
     ...
     * user
     */
    private class OrderForId implements Comparator<SaleModel> {

        /**
         * this method compare two SaleModel with the id
         *
