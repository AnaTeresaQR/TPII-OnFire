...
import views.ShowSaleApproveList;
...
//        System.out.println("tamaño lista: " + SaleModel.getSaleListManager().getWaitingApproveSalesList().size());
    ...
    public boolean createSaleBuilder(AbstractBuilderCreateSale abstractBuilderSale, String brand, String model, int year, String carId, String color, String description, int days, long minOffer) throws IOException, CustomException {
    ...
    public void showSaleApproveListView(PrincipalControllerServer controller) {
        if (showApproveList == null) {
            showApproveList = new ShowSaleApproveList(controller);

        }
        showApproveList.setVisible(true);
    }

    public void showAdministartorMenuView(PrincipalControllerServer controller, JFrame parent) {
        parent.dispose();
        if (menuServer == null) {
            menuServer = new AdministratorMenuView(controller);
        }
